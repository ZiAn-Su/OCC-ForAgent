...
If an MCP client or agent process is interrupted while an edit session is active, do not abandon the project. In a fresh MCP transport, call `get_project_agent_state` to discover the durable active session, then `open_project` for the same project and call `resume_edit_session` with its `editSessionId`. This continues the existing staged draft. If the old transport did not close cleanly, repeat with `force: true`; that is an explicit takeover. Use `recover_edit_session` only when the draft should be discarded and replaced.
